    ...
    def process_fn(
        ...
        with torch.no_grad():
            for minibatch in batch.split(self._batch, shuffle=False, merge_last=True):
                ...
                kl_losses.append(self.kl_action_smoothing(dist._dists[0].logits))
                old_log_prob.append(dist.log_prob(minibatch.act))
        batch.logp_old = torch.cat(old_log_prob, dim=0)
        batch.action_kl_loss = torch.cat(kl_losses, dim=0)
        return batch

    def kl_action_smoothing(self, action_logits: torch.Tensor, window: int = 3):
        """KL smoothing for action logits."""
        reference = action_logits.detach()
        padding = torch.cat([reference[0:1, :] for _ in range(window)], dim=0)
        reference = torch.cat([padding, reference], dim=0)
        dist1 = torch.distributions.Categorical(logits=action_logits)
        T = action_logits.shape[0]
        kl_loss = 0.0
        for w in range(window):
            dist2 = torch.distributions.Categorical(logits=reference[w:w+T, :])
            kl_loss += torch.distributions.kl_divergence(dist1, dist2)
        return kl_loss / window
    ...
    def learn(  # type: ignore
        ...
        for step in range(repeat):
            ...
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                # our action space is single categorical distribution
                # kl_loss = self.kl_action_smoothing(dist._dists[0].logits) * 1e-7
                action_kl_loss = minibatch.action_kl_loss.mean() * 1e-7

                loss = clip_loss + self._weight_vf * vf_loss \
                    - self._weight_ent * ent_loss + action_kl_loss
                #+ kl_loss
